...
### 4. (Optional) Show the clips on the Fin City Dynasty site
The bot also writes a **`media.json`** file in the repo and keeps it updated.
To make clips appear on the site's **Media** tab automatically:

1. In your GitHub repo, click `media.json`, then the **Raw** button.
2. Copy that URL (looks like
   `https://raw.githubusercontent.com/<you>/<repo>/main/media.json`).
3. In the site file, paste it into the line
   `const MEDIA_FEED_URL = '';` and redeploy.

Now every clip the bot finds posts to Discord **and** shows up on the site.
(Send that raw URL to me and I'll paste it in + hand you a ready-to-deploy build.)
...
- It records what it posted in `state.json` **and appends each clip to
  `media.json`** (newest first), then commits both back to the repo — so the
  next run doesn't repeat anything and the website always has the latest feed.
...
> Note: only clips found **from now on** appear in `media.json`. The ~22 clips
> the old bot already posted to Discord aren't backfilled (we never captured
> their details), so the site feed starts fresh.
