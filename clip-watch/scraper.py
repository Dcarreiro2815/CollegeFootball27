...
new game clips and screenshots for College Football 27 and (1) posts them to a
Discord channel via webhook and (2) appends them to media.json, which the Fin
City Dynasty website reads straight from GitHub. Already-seen items are
remembered in state.json so nothing double-posts.
...
MEDIA_FILE = Path(__file__).parent / "media.json"   # the site reads this from GitHub
...
# ── Media feed (what the website reads) ─────────────────────────────────────────
# media.json is a flat list of every clip/screenshot ever found, newest first.
# The Fin City Dynasty site fetches it straight from GitHub — no Google Sheet,
# no manual entry. Each entry: {id, kind, date, by, link, thumb}.
...
def save_media(existing: list, new_entries: list) -> None:
    """Merge new entries in, dedupe by id, sort newest-first, write."""
    by_id = {m.get("id"): m for m in existing if m.get("id")}
    for m in new_entries:
        if m.get("id"):
            by_id[m["id"]] = m
    merged = sorted(by_id.values(), key=lambda m: m.get("date") or "", reverse=True)
    MEDIA_FILE.write_text(json.dumps(merged, indent=2))


def clip_media_entry(clip: dict, gamertag: str) -> dict:
    thumbs = clip.get("thumbnails", [])
    thumb = thumbs[0].get("uri", "") if thumbs else ""
    link = ""
    for u in clip.get("gameClipUris", []):
        if u.get("fileSize", 0) > 0:
            ...
            break
    return {
        "id": clip.get("gameClipId", ""),
        "kind": "clip",
        "date": clip.get("dateRecorded", ""),
        "by": gamertag,
        "link": link,
        "thumb": thumb,
    }
...
def shot_media_entry(shot: dict, gamertag: str) -> dict:
    thumbs = shot.get("thumbnails", [])
    thumb = thumbs[0].get("uri", "") if thumbs else ""
    uris = shot.get("screenshotUris", [])
    full = uris[0].get("uri", "") if uris else ""
    return {
        "id": shot.get("screenshotId", ""),
        "kind": "shot",
        "date": shot.get("dateTaken", ""),
        "by": gamertag,
        "link": full or thumb,
        "thumb": thumb or full,
    }
...
async def scan_gamertag(
    ...
    """Scan one gamertag. Returns (newly posted IDs, new media entries)."""
    new_ids: set[str] = set()
    new_media: list[dict] = []

    xuid = await get_xuid(session, api_key, gamertag, state)
    if not xuid:
        return new_ids, new_media
    ...
    for clip in cf_clips:
        ...
        if await post_to_discord(session, webhook_url, payload, f"{gamertag}/clip:{clip_id[:8]}"):
            new_ids.add(clip_id)
            new_media.append(clip_media_entry(clip, gamertag))
    ...
    for shot in cf_shots:
        ...
        if await post_to_discord(session, webhook_url, payload, f"{gamertag}/shot:{shot_id[:8]}"):
            new_ids.add(shot_id)
            new_media.append(shot_media_entry(shot, gamertag))
        await asyncio.sleep(1)
...
async def run() -> None:
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        for gamertag in GAMERTAGS:
            try:
                new_ids, new_media = await scan_gamertag(
    ...
    save_state(state)
    save_media(media, all_media)   # always rewrite (keeps file tidy/sorted)

    if all_new:
        print(f"[DONE] Posted {len(all_new)} new item(s); media.json now has "
              f"{len(load_media())} total.")
    else:
